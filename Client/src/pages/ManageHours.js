...
const ManageHours = () => {
  ...
  const saveSchedule = () => {
    if (hotelName) {
      axios
        .post(`/save-schedule/${hotelName}`, { schedule })
        .then(() => {
      ...
      <div className="manage-hours-form">
        ...
        {shifts.map((shift) => (
          <div key={shift}>
                  ...
                  <tr key={position}>
                    ...
                    {days.map((day) => (
                      <React.Fragment key={`${shift}-${position}-${day}`}>
                        <td
                          className={
                            schedule[shift][position][day].noWeapon > 0
                              ? "selected-row"
                              : ""
                          }
                        >
                          <input
                            type="number"
                            min="0"
                            value={schedule[shift][position][day].noWeapon}
                            onChange={(e) =>
                              handleChange(
                                shift,
                                position,
                        ...
                        <td
                          className={
                            schedule[shift][position][day].weapon > 0
                              ? "selected-row"
                              : ""
                          }
                        >
                          <input
                            type="number"
                            min="0"
                            value={schedule[shift][position][day].weapon}
                            onChange={(e) =>
                              handleChange(
                                shift,
                                position,
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => removePosition(shift, position)}
                      >
                        X
                      </button>
                    </td>
            ...
            <button className="add-position" onClick={() => addPosition(shift)}>
              + Add Position
            </button>
          </div>
        ))}
        <button className="save-all" onClick={saveSchedule}>
          Save All
        </button>
      </div>
    </div>
